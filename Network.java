...
public class Network {
    ...
    public boolean addFollowee(String name1, String name2) {
        ...
        if (user1 == null || user2 == null || user1.follows(name2) || name1.equalsIgnoreCase(name2)) {
            return false;
        }
    ...
    public String recommendWhoToFollow(String name) {
        ...
        String mostRecommend = null;
        for (int i = 0; i < userCount; i++) {
            if (user.getName().equalsIgnoreCase(users[i].getName())) {
                continue;
            }
            if ((maxMutual < users[i].countMutual(user))) {
                maxMutual = users[i].countMutual(user);
                mostRecommend = users[i].getName();
            }
        }
    ...
    public String mostPopularUser() {
        ...
        for (int i = 0; i < userCount; i++) {
            if (maxCount < followeeCount(users[i].getName())) {
    ...
    public String toString() {
        //// Replace the following statement with your code
        String networkDescription = "Network:";
        for (int i = 0; i < userCount; i++) {
            networkDescription += "\n" + users[i].toString();
        }
